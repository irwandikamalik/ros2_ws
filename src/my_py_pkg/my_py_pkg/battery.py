...
class BatteryNode(Node):
    ...
    def __init__(self):
        ...
        self.battery_timer_ = self.create_timer(0.1, self.callback_check_battery_state)
    ...
    def callback_check_battery_state(self):
        ...
        if self.battery_state_ == "full":
            ...
            if time_now - self.last_time_battery_state_changed_ > 6.0:
                self.battery_state_ = "full"
                self.get_logger().info("Battery is now full. Stopping charge.")
    ...
    def callback_call_set_led(self, future):
        ...
        if response.success:
            ...
            self.get_logger().error("Failed to change LED state.")
